...
#include "soft_align/GetPath.h"
#include "soft_align/PathPoint.h"
...
// to avoid non-primary smoothing as p2p is easy to crumple when no primary action as first action
...
bool smooth(FabricState fs_c, FabricState fs_req, double allowance, GraspAndMovement &gam, vector<int> &cnr_smoothed)
{
    cnr_smoothed.clear();
    ...
    for (int i = 0; i < fs_req.cnr.size(); i++)
    {
        ...
        if (cv::norm(fs_c.cnr[i] - fs_req.cnr[i]) > allowance)
        {
            ...
            if (align_dir.dot(leftSideNormal) > 0 || align_dir.dot(rightSideNormal) > 0)
            // if (true)
            {
...
// get all gam of contact poses, only for rectangle cloth
vector<GraspAndMovement *> getAlignmentActions(FabricState fs_c, FabricState fs_req)
{
    vector<GraspAndMovement *> gam_set;
    std::vector<CornerAlign *> pullToSmooth;
    ...
    for (int i = 0; i < fs_req.cnr.size(); i++)
    {
        if (!fs_c.inView[i])
        {
            pullToSmooth.push_back(nullptr);
            pullToSmooth.push_back(nullptr);
            continue;
        }
        ...
        // if (align_dir.dot(leftSideNormal) > 0 || align_dir.dot(rightSideNormal) > 0)
        std::cout << "Corner " << i << " could be smoothed with its side!" << std::endl;
        CornerAlign *ca_l, *ca_r;
        ca_l = new CornerAlign();
        ca_r = new CornerAlign();

        ca_l->cnr_idx = i;
        ca_l->point_from = fs_c.cnr[i];
        ca_l->point_to = fs_req.cnr[i];

        cv::Point2f pick_normal, place_normal;

        ca_l->side = "L";
        ca_l->dir_from = fs_c.cnr_L[i];
        ca_l->dir_to = fs_req.cnr_L[i];
        pick_normal = leftSideNormal;
        place_normal = fs_req.getNormalOfCornerDir(i, "L");
        ca_l->pick_normal = pick_normal;
        ca_l->place_normal = place_normal;
        pullToSmooth.push_back(ca_l);

        ca_l->side = "R";
        ca_l->dir_from = fs_c.cnr_R[i];
        ca_l->dir_to = fs_req.cnr_R[i];
        pick_normal = rightSideNormal;
        place_normal = fs_req.getNormalOfCornerDir(i, "R");

        ca_l->pick_normal = pick_normal;
        ca_l->place_normal = place_normal;
        pullToSmooth.push_back(ca_r);
    }

    // for each CornerAlign in pullToSmooth, get all possible grasping and releasing poses
    for (int i = 0; i < pullToSmooth.size(); i++)
    {
        if (pullToSmooth[i] == nullptr)
        {
            gam_set.push_back(nullptr);
            continue;
        }

        GraspAndMovement *gam = new GraspAndMovement();

        cv::Point2f gp_w = pullToSmooth[i]->point_from + 0.5 * PL_LENGTH * pullToSmooth[i]->dir_from;
        cv::Point2f offset_dir = -fs_c.getNormalOfCornerDir(pullToSmooth[i]->cnr_idx, pullToSmooth[i]->side) + pullToSmooth[i]->dir_from;
        gp_w += PL_OFFSET * offset_dir;

        cv::Point2f ori_l(pullToSmooth[i]->dir_from.y, -pullToSmooth[i]->dir_from.x);
        cv::Point2f ori_r(-pullToSmooth[i]->dir_from.y, pullToSmooth[i]->dir_from.x);
        cv::Point2f gp_ori = pullToSmooth[i]->pick_normal;

        cv::Point2f rp_w = pullToSmooth[i]->point_to + 0.5 * PL_LENGTH * pullToSmooth[i]->dir_to;
        offset_dir = -fs_req.getNormalOfCornerDir(pullToSmooth[i]->cnr_idx, pullToSmooth[i]->side) + pullToSmooth[i]->dir_to;
        rp_w += PL_OFFSET * offset_dir;

        ori_l = cv::Point2f(pullToSmooth[i]->dir_to.y, -pullToSmooth[i]->dir_to.x);
        ori_r = cv::Point2f(-pullToSmooth[i]->dir_to.y, pullToSmooth[i]->dir_to.x);
        cv::Point2f rp_ori = pullToSmooth[i]->place_normal;
        ...
        gam_set.push_back(gam);
    }

    // get gam of middle points of four side
    int num = fs_req.cnr.size();
    for (int i = 0; i < fs_req.cnr.size(); i++)
    {
        GraspAndMovement *gam = new GraspAndMovement();
        ...
        cv::Point2f gp_w = (pullToSmooth[i * 2]->point_from + pullToSmooth[((i + 1) % num) * 2]->point_from) / 2;
        ...
        gam->pick.orientation = gam_set[i * 2]->pick.orientation;

        cv::Point2f rp_w = (pullToSmooth[i * 2]->point_to + pullToSmooth[((i + 1) % num) * 2]->point_to) / 2;
        ...
        gam->place.orientation = gam_set[i * 2]->place.orientation;

        gam_set.push_back(gam);
    }

    vector<GraspAndMovement *> push_pose_gam;

    vector<int> order = {0, 8, 3, 2, 9, 5, 4, 10, 7, 6, 11, 1};
    for (int i = 0; i < order.size(); i++)
    {
        // dereference gam_set[order[i]] and push back
        push_pose_gam.push_back(gam_set[order[i]]);
    }

    return push_pose_gam;
}

// -1: no primary motion
void reposition2pos(Manipulator manipulator, FabricState fs_to, int num_marker, ros::ServiceClient uiService, bool uiUpdate = true, int primary_action = -1, bool refine = false)
{
    if (primary_action != -1)
    {
        ...
        if (uiUpdate)
        {
            ...
            // print size of fs_to.ctr_x and fs_to.cnr_x
            cout << "fs_to.ctr_x size: " << fs_to.ctr_x.size() << endl;
            // print ctr_x
            for (int i = 0; i < fs_to.ctr_x.size(); i++)
            {
                cout << "fs_to.ctr_x[" << i << "]: " << fs_to.ctr_x[i];
            }
            cout << endl;
            cout << "fs_to.cnr_x size: " << fs_to.cnr_x.size() << endl;
            // print cnr_x
            for (int i = 0; i < fs_to.cnr_x.size(); i++)
            {
                cout << "fs_to.cnr_x[" << i << "]: " << fs_to.cnr_x[i];
            }
            ...
            // cnr_smoothed initilized to true
            vector<int> cnr_smoothed(fs_to.cnr_x.size(), 0);
        ...
        // 1. Primary action
        // primary_action corresponds to a contact pose for the first action
        FabricState fs_c = get_state_from_camera(num_marker);
        // get current contact poses
        vector<GraspAndMovement *> pullToAlign = getAlignmentActions(fs_c, fs_to);

        GraspAndMovement *primary = pullToAlign[primary_action];
        if (primary == nullptr)
        {
            cout << "Primary action is invisible!" << endl;
            return;
        }

        // Do smoothing control
        double pos_s[3] = {primary->pick.sticking_point.x, primary->pick.sticking_point.y, primary->pick.sticking_point.z + STICKING_HIGHT - GRIPPER_OFFSET};
        double dir_s[3] = {primary->pick.orientation.x, primary->pick.orientation.y, 0};
        cout << "Place the push line with the required position and orientation." << endl;
        manipulator.grasp(dir_s, pos_s);
        double pos_rls[3] = {primary->place.sticking_point.x, primary->place.sticking_point.y, primary->place.sticking_point.z + STICKING_HIGHT - GRIPPER_OFFSET};
        double dir_rls[3] = {primary->place.orientation.x, primary->place.orientation.y, 0};
...
int main(int argc, char **argv)
{
    ...
    ros::ServiceClient pathClient = node.serviceClient<soft_align::GetPath>("get_path");
    ...
    // get the zero pose of the cloth
    FabricState fs_zero;
    fs_start.copyTo(fs_zero);
    ft.transform(fs_zero, Point2f(-fs_zero.center.x, -fs_zero.center.y), fs_zero.center, -fs_zero.ori);
    ...
    if (use_primary)
    {
        ...
        // get init and target state, and do the planning
        // fs_start, fs_end
        cv::Point3f start_pos = fs_start.getRigidForm();
        cv::Point3f end_pos = fs_end.getRigidForm();
        //
        // execute the planning
        soft_align::GetPath srv; // Replace 'your_package_name' with the actual name of your package

        srv.request.start_x = start_pos.x;
        srv.request.start_y = start_pos.y;
        srv.request.start_theta = start_pos.z;
        srv.request.target_x = end_pos.x;
        srv.request.target_y = end_pos.y;
        srv.request.target_theta = end_pos.z;

        if (pathClient.call(srv))
        {
            for (const auto &point : srv.response.path)
            {
                ROS_INFO("Received Path Point: (%f, %f, %f) with action %ld", point.x, point.y, point.theta, point.action);
                // move cloth to point with action
                FabricState subgoal;
                fs_zero.copyTo(subgoal);
                // print zero pose, print every corner point of zero
                // for (int i = 0; i < fs_zero.cnr.size(); i++)
                // {
                //     cout << "zero pose corner " << i << ": " << fs_zero.cnr[i] << endl;
                // }

                
                ft.transform(subgoal, Point2f(point.x, point.y), subgoal.center, point.theta);

                // print sub goal, every corner
                // cout << "subgoal: " << subgoal.center << endl;
                // for (int i = 0; i < subgoal.cnr.size(); i++)
                // {
                //     cout << "subgoal corner " << i << ": " << subgoal.cnr[i] << endl;
                // }

                // print position of cnr_x
                for (int i = 0; i < subgoal.cnr_x.size(); i++)
                {
                    cout << "subgoal cnr_x " << i << ": " << subgoal.cnr_x[i] << endl;
                }

                // move to subgoal
                reposition2pos(manipulator, subgoal, num_marker, uiClient, true, point.action, false);
            }
        }
        else
        {
            ROS_ERROR("Failed to call service get_path");
            return 1;
        }
    }
    ...
    reposition2pos(manipulator, fs_end, num_marker, uiClient, true, -1, true);
